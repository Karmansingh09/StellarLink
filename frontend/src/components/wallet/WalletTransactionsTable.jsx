...
export default function WalletTransactionsTable({ publicKey }) {
  ...
  const activeKey = publicKey || 'GBHPLJTE52JPNNGRU7W5JCKSV3JYFS5ZNMF27IQDTTPDGSP3XRZYCHFE';

  const { data: transactions = [], isLoading } = useStellarTransactions(activeKey);

  const walletTxs = transactions.map((tx) => ({
    ...
    <Card padding="generous">
      ...
      {walletTxs.length === 0 ? (
        <div className="py-8 text-center space-y-2 border border-dashed border-[#E2E8F0] rounded-2xl bg-[#F8FAFC]">
          <Clock className="h-8 w-8 text-[#64748B] mx-auto" />
          <p className="text-sm font-semibold text-[#0F172A]">No Recent Ledger Transactions</p>
          <p className="text-xs text-[#64748B]">No on-chain activity recorded on Stellar Horizon for this wallet address.</p>
        </div>
      ) : (
        <>

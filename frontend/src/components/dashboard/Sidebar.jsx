...
export default function Sidebar({ navItems }) {
  ...
  return (
    <>
      ...
      <aside className="sticky top-0 hidden h-screen w-72 shrink-0 border-r border-[#E2E8F0]/80 bg-white/90 px-5 py-6 backdrop-blur-xl lg:flex lg:flex-col justify-between overflow-y-auto">
        <div>
                  ...
                  >
                    {({ isActive }) => (
                      ...
                      >
                        <span
                          className={clsx(
                            'flex h-9 w-9 items-center justify-center rounded-[12px] border transition-colors shrink-0',
                            isActive
                        ...
                        {item.end !== true && <Wifi className="h-4 w-4 text-[#CBD5E1] shrink-0" />}
                      </motion.div>
                    )}
                  </NavLink>
                );
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  clsx(
                    'flex flex-col items-center justify-center gap-0.5 rounded-[12px] py-1 text-[9px] xs:text-[10px] font-semibold transition-colors select-none min-w-0',
              ...
              >
                {({ isActive }) => (
                  <motion.div
                    whileTap={{ scale: 0.92 }}
                    className="flex flex-col items-center justify-center gap-0.5 w-full text-center min-w-0"
                  >
                    <Icon className={clsx('h-4 w-4 shrink-0', isActive ? 'text-[#0F766E]' : 'text-[#64748B]')} />
                    <span className="truncate w-full block text-center px-0.5 leading-tight">{item.label}</span>
                  </motion.div>
                )}
              </NavLink>
            );
